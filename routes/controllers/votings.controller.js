...
exports.createVoting = async function (req, res, next) {
  ...
  try {
    ...
    if (!title) {
      throw createError(400, ERROR.INVALID_TITLE);
    }

    if (!validateDate(expiredAt)) {
      throw createError(400, ERROR.INVALID_EXPIRATION_DATE);
    }
    ...
    if (!options) {
      throw createError(400, ERROR.INVALID_VOTING_OPTION);
    }

    if (options.length < MINIMUM_NUMBER_OF_OPTIONS) {
      throw createError(400, ERROR.NOT_ENOUGH_OPTIONS_INPUT);
    }
    ...
    if (err instanceof mongoose.Error.ValidationError) {
      res.render("votingCreation/failure", { message: ERROR.INVALID_DATA })
    }
...
exports.getDetails = async function (req, res, next) {
  try {
    ...
    if (!mongoose.isValidObjectId(votingId)) {
      return next(createError(400, ERROR.INVALID_VOTING_ACCESS));
    }
    ...
    if (!voting) {
      return next(createError(404, ERROR.NOT_FOUND));
    }
...
exports.vote = async function (req, res, next) {
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(votingId)) {
      throw createError(400, ERROR.INVALID_VOTING);
    }
